...
public:
    ...
    int interval(T &lower, T &upper) {
        Link root = this->raiz;
        return count_interval(lower, upper, root);
    }
...
protected:
    ...
    int count_interval(T &lower, T &upper, Link nodo) {
        int cont = 0;
        if (nodo == nullptr) {
            return 0;
        }
        else {
            if(!menor(nodo->elem, lower) && !menor(upper,nodo->elem)) {
                cont++;
            }
            if (menor(lower,nodo->elem)) {
                cont += count_interval(lower, upper, nodo->iz);
            }
            if (menor(nodo->elem,upper)) {
                cont += count_interval(lower, upper, nodo->dr);
            }
        }
        return cont;
    }
